...
<div class="container rounded shadow bg-white">
    <!-- <div class=""> -->
        <div class="row mt-n6 p-2" id="lightGallery">
            <?php 
            $decoupe = explode(';', $station->galery);
            $i = 0;
            foreach ($decoupe as $value) {
                if($value != ""){ ?>
                    <div class="col-12 col-md-4 contentimg" data-src="http://anmsm.media.tourinsoft.eu/upload/<?php echo $value; ?>">
                        <img src="#" data-src="http://anmsm.media.tourinsoft.eu/upload/<?php echo $value; ?>" class="img-fluid imageclass rounded w-100" />
                    </div>
            <?php $i++; }
            }
            if($i == 0){ ?>
            <div class="col-12 p-5">
                <h1 class="text-center"><?= __("Pas de galerie à afficher pour cette station") ?></h1>
            </div>
            <?php } ?>
        </div>   
        <?php if(count($decoupe) > 10){ ?>
            <div class="row">
...
<script src="<?php echo $this->Url->build('/')?>js/new/lightgallery-all.min.js"></script>
