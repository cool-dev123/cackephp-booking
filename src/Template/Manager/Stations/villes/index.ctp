        ...
        <div class="panel panel-default card-view">
            <div class="panel-heading icantSelectIt">
                <div class="pull-left mt-10">
                        <h6 class="panel-title txt-dark">Villes</h6>
                </div>
                <div class="pull-right">
                    <a href="<?php echo $this->Url->build('/',true);?>manager/ville/add/" class="btn btn-primary pull-right">Ajouter Ville</a>
                </div>
...
//<script>
    var table;
    $(document).on('click', ".delete_ville", function () {
        var id = $(this).attr("data-key");
        swal({
            title: "Suppression d\'un Ville",   
            ...
            $.ajax({
            type: "delete",
            url: "<?php echo $this->Url->build('/',true)?>/manager/ville/delete/"+id,

            success:function(xml){
                $('body').loadingModal('destroy');
                if(xml=='deleted'){
                    swal("", "Vous venez de supprimer une ville", "success");
                    table.ajax.reload();
                }
                else{
                    swal({
                        title: "Erreur de suppression",   
                        text: "Cette ville contient des villages, Vous ne pouvez pas le supprimer",   
                        type: "error",
    ...
    $(document).ready(function() {
        table=$('#datable_1').DataTable({
            responsive: true,
            "ajax":{
                "url": "<?= $this->Url->build('/',true)?>manager/ville/allvilles",
            },
