        ...
        <div class="panel panel-default card-view">
            <div class="panel-heading icantSelectIt">
                <div class="pull-left mt-10">
                        <h6 class="panel-title txt-dark">Liste Règles Paiement</h6>
                </div>
                <div class="pull-right">
                    <a href="<?php echo $this->Url->build('/',true);?>manager/paiements/add/" class="btn btn-primary pull-right">Ajouter Règle Paiement</a>
                </div>
...
//<script>
    ...
    $(document).on('click', ".delete_paiement", function () {
        ...
        $.ajax({
            type: "POST",
            dataType : 'json',
            url: "<?php echo $this->Url->build('/',true)?>manager/paiements/getproppaiementrelation/"+id,
            success:function(xml){
                if(xml == "existe") textsupp = "Attention, cette règle paiement est liée à un ou plusieurs propriétaires de résidences. Si vous supprimez cette règle, la règle de paiement du système actuelle sera appliquée.";
                swal({
                    title: "Êtes-vous sûr de vouloir supprimer cette règle Paiement ?",   
                    text: textsupp,   
                    type: "warning",   
                    ...
                    $.ajax({
                    type: "delete",
                    url: "<?php echo $this->Url->build('/',true)?>/manager/paiements/deletepaiement/"+id,

                    success:function(xml){
                        $('body').loadingModal('destroy');
                        if(xml=='deleted'){
                            swal("", "Vous venez de supprimer une paiement", "success");
                            table.ajax.reload();
                        }else{
                            swal({
                                title: "Erreur de suppression",   
                                text: "Vous ne pouvez pas supprimer cette paiement",   
                                type: "error",
    ...
    $(document).ready(function() {
        ...
        table=$('#datable_1').DataTable({
            responsive: true,
            "ajax":{
                "url": "<?= $this->Url->build('/',true)?>manager/paiements/allpaiements",
            },
