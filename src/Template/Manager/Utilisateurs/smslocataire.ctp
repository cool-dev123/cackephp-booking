...
function verfier_champ(){
	...
	if(msg.length>0){
        ...
        <div class="widget-header">
            <span><i class="icon-table"></i> Envoi sms depuis alpissime.com </span>
			...
			<form action="" method="post" enctype="multipart/form-data" id="frm_send-sms" onsubmit="return verfier_champ()" >
				<fieldset>
					<table width='100%'>
							...
							<tbody>
							  <? $cpt=0;foreach($a_list_loc as $enr) {?>
								<tr <?php if($cpt%2==0){?>style="background-color:#E4E4E4" <?php }?>>
									<td>
										<input type="checkbox" id="check_<?php echo $enr['U']['id']?>" value="<?php echo str_replace(" ","",$enr['U']['portable'])?>_<?php echo $enr['U']['id']?>" name="data[sel][]" >
									</td>
									<td><?=$enr['U']['prenom']?></td>
									<td><?=$enr['U']['nom_famille']?></td>
									<td><?=$enr['U']['ville']?></td>
									<td><?=$enr['U']['portable']?></td>
						...
						<div class="section">
							<label for="PackSujet">Portable :</label>
							<input id="smstelephone" type="text" value="" style="width:261px;" readOnly name="data[telephone]"/>
						...
						<div class="section">
							<label for="PackSujet">Model sms :</label>
								...
								<div><input type="radio" name="data[model]" value="<?php echo $m['Modelmessage']['id']?>"/><?php echo $m['Modelmessage']['titre']?></div>
								<?php endforeach;?>
						</div>
						
						<div class="submit"><input type="submit" value="Envoyer" style="margin-top:10px" class="uibutton icon add"><a href="javascript:void(0)" onclick="reset_frm()" class="uibutton special answer"> Reset </a></div>
